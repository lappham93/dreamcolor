...
//				parent.ul.appendChild(node.li);
	...
	nodeRenderTitle: function(ctx, title) {
		...
		if( opts.checkbox && node.hideCheckbox !== true && !node.isStatusNode() ) {
			if(aria){
				ares.push("<span role='checkbox' class='fancytree-checkbox' style='display:none'></span>");
			}else{
				ares.push("<span class='fancytree-checkbox' style='display:none'></span>");
			}
		}
		...
		if( icon !== false ) {
			...
			if ( typeof icon === "string" ) {
				if( TEST_IMG.test(icon) ) {
					...
					if (!node.expanded && "childCount" in node.data && node.data.childCount > 0) {
						ares.push("<span " + role + " class='fancytree-custom-icon " + icon +  "'><span class='fancytree-childcounter' style='color:white; margin-top:0px; padding:0px'/>" + node.data.childCount +"</span></span>");
					}else {
				...
				if (!node.expanded && "childCount" in node.data && node.data.childCount > 0) {
					ares.push("<span " + role + " class='fancytree-icon'><span class='fancytree-childcounter' style='color:white; margin-top:0px; padding:0px'/>" + node.data.childCount +"</span></span>");
				}else {
		...
		if(!nodeTitle){
			...
			nodeTitle = "<span " + role + " class='fancytree-title'" + id + tooltip + tabindex + ">" + node.title + "<span class='hidden-480'> [" + node.data.name + "]</span>" + "</span>";
		}
		ares.push(nodeTitle);
		...
		tree.$container.on("focusin" + ns + " focusout" + ns, function(event){
			...
			try {
				...
				case "click":
					...
					if (ctx.options.clickHandle != null) {
						ctx.options.clickHandle(node.data);
					}
