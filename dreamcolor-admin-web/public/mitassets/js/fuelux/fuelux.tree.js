...
(function (factory) {
	...
	Tree.prototype = {
		...
		populate: function ($el) {
			var role_icon = []
			role_icon[6] = "ace-icon fa fa-trophy";
			role_icon[5] = "ace-icon fa fa-life-ring";
			role_icon[4] = "ace-icon fa fa-cubes";
			role_icon[3] = "ace-icon fa fa-star";
			role_icon[2] = "ace-icon fa fa-star-half-o";
			role_icon[1] = "ace-icon fa fa-star-o";
			role_icon[0] = "ace-icon fa fa-spinner";
			var self = this;
			...
			this.options.dataSource( treeData ? treeData : {} , function (items) {
				...
				$.each( items.data, function(index, value) {
					...
					//add icon
					var icon = (value.role >= 1 && value.role <= 6) ? role_icon[value.role] : role_icon[0];
					var text = '<i class="' + icon + '"></i>';
					text += " " + value.text;

					if(value.type === 'folder') {
						$entity = self.$element.find('[data-template=treebranch]:eq(0)').clone().removeClass('hide').removeAttr('data-template');
						$entity.data(value);
						$entity.find('.tree-branch-name > .tree-label').html(text);
						...
						$entity.find('.tree-item-name > .tree-label').html(text);
						$entity.data(value);
