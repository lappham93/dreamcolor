 ...
 * @since Jan 26, 2016
 */
public class ProductPhotoServlet extends BaseHandler {
    private static Logger logger = LoggerFactory.getLogger(ProductPhotoServlet.class);
    private final int minSize = 20;
    
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) {
        try {
            renderProductPhoto(req, resp);
        } catch (Exception ex) {
...
//                }
    ...
    private void renderProductPhoto(HttpServletRequest req, HttpServletResponse resp){
        try {
            ...
            if(p != null && !p.isEmpty()){
                //logger.info("ProductPhotoServlet load - pid: " + p);
                ...
                if(tptrs != null && tptrs.error >= 0 && tptrs.value != null){
                    ...
                    logger.info("ProductPhotoServlet load from rockdb - pid: " + p + " Size: " + size + " FileSize: " + FileUtils.byteCountToDisplaySize(data.length));
                    if(data != null && size > 0){
                    ...
                    logger.info("ProductPhotoServlet after resizeImage - pid: " + p + " Size: " + size + " FileSize: " + FileUtils.byteCountToDisplaySize(fileSize));
                    ...
                    logger.info("ProductPhotoServlet load from rockdb - pid: " + p + " Size: " + size + " -------> NULL ");
            ...
            logger.info("ProductPhotoServlet - pid: " + p + " Size: " + size + " FileSize: " + FileUtils.byteCountToDisplaySize(fileSize) + " Time: " + (System.nanoTime() - t) + "ns ");
        } catch (Exception e) {
            logger.error("renderProductPhoto " + e.getMessage(), e);
        }
    }
}
