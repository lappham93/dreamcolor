...
public abstract class ServletWrapper extends HttpServlet {
	...
	public void printJson(HttpServletRequest req, ServletResponse resp, String json) throws IOException {
		...
		writer.write(json);
		writer.close();
		logger.debug(req.getAttribute("uuid") + "\tEND_REQUEST\t" + req.getMethod() + "\t" + req.getRequestURI() + "\t" 
				+ json);
	}
	...
	public boolean checkNullableParameter(Map<String, Object> paramValues, List<String> params) {
		if(paramValues == null) {
			return false;
		}
