...
//					return;
//				}
//				// Nếu đã login và không phải là Admin thì check quyền truy suất
//				// url.
//				if ((!isAdmin(req)) && (!isBizAdmin(req) || !Common.bizAdmin.contains(subPath))
//						&& (!isCNDAdmin(req) || !Common.CNDAdmin.contains(subPath))) {
//					renderDenyPage(req, resp);
//					return;
//					// //Nếu không truy suất vào các url đc phép thì render page
//					// deny.
...
//			}
	...
	protected void printFile(HttpServletResponse response, ByteArrayOutputStream bigfile, String fileName,
	...
	protected String getContentTemplate(String tplName, HttpServletRequest req) {
		try {
			...
			logger.error("getContentTemplate: " + ex, ex);
		}
		return "";
	}
	...
	protected String applyTemplate(TemplateDataDictionary dic, String tplName, HttpServletRequest req) {
		try {
			...
			// req.getSession().getAttribute("accountInfo");
			...
			logger.error("applyTemplate: " + ex, ex);
		}
		return "";
	}

	protected String applyTemplateLayoutMain(TemplateDataDictionary dic, HttpServletRequest req,
		...
		try {
			...
			if ("/web/dkm".equalsIgnoreCase(svl)) {
				dic.setVariable("MN_HOME", "active");
			} else if ("/web/dkm/home".equalsIgnoreCase(svl)) {
				dic.setVariable("MN_HOME", "active");
			} else if ("/web/dkm/colorchart".equalsIgnoreCase(svl)) {
				dic.setVariable("MN_CC", "active");
			} else if ("/web/dkm/aboutus".equalsIgnoreCase(svl)) {
				dic.setVariable("MN_ABOUT", "active");
			} else if ("/web/dkm/contact".equalsIgnoreCase(svl)) {
			...
			logger.error("applyTemplateLayoutMain: " + ex, ex);
		}
		return "";
	}

	protected String applyTemplateLayoutEdu(TemplateDataDictionary dic, HttpServletRequest req,
			HttpServletResponse resp) {
		try {
			Template template = Loader.getTemplate("layoutEdu.xtm");
			dic.setVariable("Project-Title", Configuration.APP_TITLE);
			dic.setVariable("domain", Configuration.APP_DOMAIN);
			dic.setVariable("static-domain", Configuration.APP_STATIC_DOMAIN);
			...
			logger.error("applyTemplateLayoutEdu: " + ex, ex);
		}
		return "";
	}
