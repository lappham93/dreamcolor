 ...
 * @since Jan 9, 2016
 */
public class BannerNewsPhotoClient {
    private static final Class _ThisClass = BannerNewsPhotoClient.class;
	private static final Logger _Logger = LoggerFactory.getLogger(_ThisClass);
	private static final String _name = "newsphoto";
	private TClientPool.BizzConfig _bizzCfg;
    
    private static BannerNewsPhotoClient instance = new BannerNewsPhotoClient();
    
    private BannerNewsPhotoClient(){
        assert (_name != null && !_name.isEmpty());
		_initialize();
    }
    
    public static BannerNewsPhotoClient getInstance(){
        return instance;
    }

	private void _initialize() {
        System.out.println("BannerNewsPhotoClient._initialize");
	...
	public int ping() {
		...
		try {
            ...
            _Logger.error("BannerNewsPhotoClient.ping", ex);
    ...
    public int putMPhoto(TMPhoto tmp) {
		...
		try {
			...
			pool.invalidateClient(cli, ex);
            _Logger.error("BannerNewsPhotoClient.putMPhoto", ex);
			return MErrorDef.BAD_CONNECTION;
		} catch (TException ex) {
			pool.invalidateClient(cli, ex);
			_Logger.error("BannerNewsPhotoClient.putMPhoto", ex);
    ...
    public int multiPutMPhoto(List<TMPhoto> list) {
		...
		try {
			...
			pool.invalidateClient(cli, ex);
            _Logger.error("BannerNewsPhotoClient.multiPutMPhoto", ex);
			return MErrorDef.BAD_CONNECTION;
		} catch (TException ex) {
			pool.invalidateClient(cli, ex);
			_Logger.error("BannerNewsPhotoClient.multiPutMPhoto", ex);
    ...
    public void owPutMPhoto(TMPhoto tmp) {
		...
		try {
			...
			pool.invalidateClient(cli, ex);
            _Logger.error("BannerNewsPhotoClient.owPutMPhoto", ex);
		} catch (TException ex) {
			pool.invalidateClient(cli, ex);
			_Logger.error("BannerNewsPhotoClient.owPutMPhoto", ex);
		}
    ...
    public TMPhotoResult getMPhoto(long mvId) {
		...
		for (int retry = 0; retry < bCfg.getNRetry(); ++retry) {
			...
			try {
				...
				pool.invalidateClient(cli, ex);
                _Logger.error("BannerNewsPhotoClient.getMPhoto", ex);
				continue;
			} catch (TException ex) {
				pool.invalidateClient(cli, ex);
                _Logger.error("BannerNewsPhotoClient.getMPhoto", ex);
